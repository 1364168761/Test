...
from maze_solver import bfs_maze
from maze_solver import dfs_maze
...
def random_maze_size(): #size：奇数 <- 地图最外围默认是一圈墙，迷宫单元在地图中的位置(X,Y），比如（1，1），（5，9）都是奇数，可以表示成(2 * x+1, 2 * y+1)，取值范围
...
if __name__ == '__main__':
    ...
    SOLVE_THREAD = threading.Thread(target=solve_maze, args=(MAZE, ENTRANCE, EXIT, draw_maze))  #解迷宫线程
    SOLVE_THREAD.start()
    while True:
        CLOCK.tick(FPS)
